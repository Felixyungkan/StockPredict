...
def get_stock_data():
    ...
    df_filter = df_reset.dropna(subset=['Date', 'Close_GOOG'])
    ...
    plt.scatter(df_filter['Date'], df_filter['Close_GOOG'], color='blue', label="Actual Data", alpha=0.6)

    plt.plot(date_range, y_pred, color='red', label="Linear Regression", linewidth=2)
    plt.axhline(df_filter['Close_GOOG'].mean(), color='green', linestyle='dashed', linewidth=2, label="Average Price")

    plt.xlabel("Date")
    plt.ylabel("Close Price GOOG (USD)")
    ...
    output_path = os.path.join(output_dir, "GOOG.png")
